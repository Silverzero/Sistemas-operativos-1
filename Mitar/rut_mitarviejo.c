...
int createTar(int nFiles, char *fileNames[], char tarName[]) {
  ...
  int escrito;
  //variables para guardar los strings de cabeceras y los tamaños de los archivos
  ...
  for(i = 0;i < nFiles; i++)
  {
    ...
    //si no da error guardo el tamaño de lo copiado para rellenar luego la cabecera
    if (escrito<0)
    {
      goto error;
    }
